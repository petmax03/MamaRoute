...
env.backends.onnx.wasm.wasmPaths = './libs/ort';

const MODEL = "qwen25-0.5b-instruct";
...
const FALLBACK={ support:['Давай поставим тишину вокруг и вдохнём медленно — ты рядом, и это уже хорошо.','Запах яблок с мёдом помнишь? Так же мягко — и с собой сейчас.','Мы брали лестницы вместе; и это возьмём. Шаг за шагом.'],
                 focus:['Один добрый шаг: тёплый чай, три медленных вдоха — и маленькое дело.','Запишем одну мысль в блокнот — остальное подождёт.','Откроем окно и выберем одну вещь сделать спокойно.'],
                 thanks:['Спасибо за хлеб дома и за дорогу в школу — это со мной всегда.','Спасибо за английский и тепло рук — ты в каждом моём шаге.','Спасибо за путь. Люблю и помню.'] };
...
function crop2(s){ const m=s.match(/[^.!?]+[.!?]/g); if(!m) return s.slice(0,220).trim();
  let out=(m[0]||'').trim(); if(m[1]) out+=' '+m[1].trim();
  if(out.length>220) out=out.slice(0,220).replace(/[,;:\-\s]+[^,.;:\-\s]*$/,'').trim()+'…'; return out; }
...
document.getElementById('confetti')?.addEventListener('click',()=>{ const old=outEl?.textContent||''; if(!outEl) return;
  outEl.textContent=(old?old+' ':'')+'♡'; setTimeout(()=>{ outEl.textContent=old; },900); });

const gal=document.getElementById('gallery');
if (gal){ for(let i=1;i<=9;i++){ const img=new Image(); img.src=`./img/${i}.jpg`; img.loading='lazy'; img.onerror=()=>img.remove(); img.onload=()=>gal.appendChild(img); img.alt='Наш кадр'; } }

const note=document.getElementById('note'); const KEY='mamaroute_note'; if(note) note.value = localStorage.getItem(KEY) || '';
document.getElementById('saveNote')?.addEventListener('click',()=>localStorage.setItem(KEY,note?.value||''));
document.getElementById('exportNote')?.addEventListener('click',()=>{ const b=new Blob([JSON.stringify({note:note?.value||'',at:new Date().toISOString()},null,2)],{type:'application/json'});
  const a=document.createElement('a'); a.href=URL.createObjectURL(b); a.download='mamaroute_note.json'; a.click(); URL.revokeObjectURL(a.href); });

let deferredPrompt=null; addEventListener('beforeinstallprompt',(e)=>{ e.preventDefault(); deferredPrompt=e; });
document.getElementById('install')?.addEventListener('click',async()=>{ if(deferredPrompt){ deferredPrompt.prompt(); deferredPrompt=null; } });
...
let generatorPromise=(async()=>{
  try{
    ...
    console.warn('[transformers] load failed:', e);
...
document.getElementById('copy') ?.addEventListener('click',async()=>{ const t=outEl?.textContent?.trim(); if(!t) return;
