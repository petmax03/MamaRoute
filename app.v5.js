// --- ENV: только локальные модели + локальные ORT .wasm ---
...
if (env) {
  ...
  // Подсказываем, где лежат wasm-файлы ORT:
  env.backends ??= {};
  env.backends.onnx ??= {};
...
// --- Мини-контент (оставляю как было, без комментариев) ---
...
if (gal) {
  [...Array(9)].forEach((_,i)=>{
    const img = new Image();
    img.src = `./img/${i+1}.jpg`; img.loading = 'lazy';
...
document.getElementById('exportNote')?.addEventListener('click', ()=>{
  ...
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'mamaroute_note.json'; a.click(); URL.revokeObjectURL(a.href);
});
...
const PERSONA = `Ты — маленький офлайн-помощник для мамы. Пиши по-русски. 1–2 предложения, тепло, без назиданий и вопросов. Без медицинских советов. Простыми словами.`;
const FACTS = [
...
const FALLBACK = {
  support: ['Давай поставим тишину вокруг и вдохнём медленно — ты рядом, и это уже хорошо.','Запах яблок с мёдом помнишь? Так же мягко — и с собой сейчас.','Мы брали лестницы вместе; и это возьмём. Шаг за шагом.'],
...
// --- выбираем модель (Qwen 0.5B Instruct локально) ---
const MODEL_ID = 'qwen25-0.5b-instruct'; // папка в ./models
...
let generatorPromise = (async () => {
  ...
  try {
    ...
    const gen = await pipeline('text-generation', MODEL_PATH);
    clearInterval(tick); barTo(100); setTimeout(()=>{ if(loadWrap) loadWrap.style.display='none'; }, 300);
    console.log('[OK] pipeline ready:', MODEL_PATH);
    return gen;
  } catch (e) {
    console.error('[ERR] pipeline:', e);
...
async function generate(mode){
  ...
  try{
    ...
    const out = await gen(prompt, { max_new_tokens: 64, temperature: 0.7, top_p: 0.9, repetition_penalty: 1.1, do_sample: true });
    ...
    console.warn('fallback:', e?.message||e);
...
// На время отладки отключим регистрацию SW (если она у тебя была в старом файле, просто не копируем её сюда).
