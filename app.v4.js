...
const gal=document.getElementById('gallery'); if(gal){for(let i=1;i<=9;i++){const img=new Image();img.src=`./img/${i}.jpg`;img.loading='lazy';img.onerror=()=>img.remove();img.onload=()=>gal.appendChild(img);img.alt='Наш кадр';}}
...
let generatorPromise=(async()=>{
  ...
  try{
    ...
    const gen = await pipeline('text-generation', window.__MODEL_PATH);
    clearInterval(t);barTo(100);setTimeout(()=>{if(loadWrap)loadWrap.style.display='none'},300);
    return gen;
  }catch(e){console.warn('[no-model]',e); if(loadWrap) loadWrap.style.display='none'; return null;}
})();
