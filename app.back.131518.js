(function(){
const T=window.transformers||{}; if(T.env){
...
window.__MODEL_PATH="./models/qwen25-0.5b-instruct";
})();
/* === минимальный офлайн-помощник + Qwen === */
...
const gal=document.getElementById('gallery'); if(gal){[...Array(9)].forEach((_,i)=>{const img=new Image();img.src=`./img/${i+1}.jpg`;img.loading='lazy';img.onerror=()=>img.remove();img.onload=()=>gal.appendChild(img);img.alt='Наш кадр';});}
...
/* не регистрируем SW на localhost, чтоб не мешал разработке */
...
if(env){env.allowLocalModels=true;env.allowRemoteModels=false;env.localModelPath='./models';env.useBrowserCache=true;}
...
let generatorPromise=(async()=>{
  ...
  try{
    ...
    const gen=await pipeline('text-generation','qwen25-0.5b-instruct'); // <— локальная папка в ./models
